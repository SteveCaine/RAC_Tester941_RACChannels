...
+ (double)			   ageOfFile:(NSString *)path error:(NSError **)outError;
...
+ (NSUInteger)countForFilesOfType:(NSString *)type inDir:(NSString *)path filter:(BOOL(^)(NSString *))filter;
+ (NSUInteger)   clearFilesOfType:(NSString *)type inDir:(NSString *)path filter:(BOOL(^)(NSString *))filter;

+ (NSArray *)pathsForFilesType:(NSString *)type inDir:(NSString *)path sortedBy:(FilesUtil_SortFilesBy)sortedBy;
